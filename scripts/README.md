...
| `node scripts\security-tests\e2e-home.mjs` | إدارة الصفحة الرئيسية: صلاحيات الفريق، البنرات (روابط داخلية فقط، صور من التخزين، الجدولة، استهداف المحافظة، الترتيب، النقرات)، ترتيب الأقسام وإخفاؤها، المتاجر المميزة بالباقات، سجل التدقيق | 34 |
| `node scripts\security-tests\e2e-orders.mjs` | الطلبات والسلة: اشتراط تسجيل الدخول، سلة لكل متجر ومنع خلط متجرين، الكمية والسعر، التوصيل مقابل الاستلام وطرق الدفع، العنوان، منع التكرار، تأكيد التاجر مع أجرة التوصيل، إلغاء الزبون، وعزل الطلبات بين المتاجر | 32 |
| `node scripts\security-tests\e2e-posts.mjs` | نشر المحلات: صلاحية التاجر، رفض الصور من خارج المنصة، شروط كل نوع (منشور/ريل/حالة)، ربط المنتج بالمتجر نفسه، ظهور المنشور في الواجهة والحالة في الدوائر واختفاؤها بعد يوم، العدادات، والإخفاء والحذف وعزل المتاجر | 21 |
