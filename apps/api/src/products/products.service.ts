...
export class ProductsService {
  ...
  async list(query: Record<string, string>) {
    ...
    const ranked = query.q?.trim()
      ? await this.search.rankProducts(query.q)
      : null;
    if (ranked) {
      ...
      const byScore = (a: { id: string }, b: { id: string }) =>
        ranked.scores.get(b.id)! - ranked.scores.get(a.id)!;
      const sorters: Record<
        string,
        (a: (typeof matches)[number], b: (typeof matches)[number]) => number
      > = {
        newest: (a, b) => b.createdAt.getTime() - a.createdAt.getTime(),
        popular: (a, b) =>
          b.contactsCount - a.contactsCount || b.viewsCount - a.viewsCount,
      ...
      const items = pageIds
        ...
        .filter(Boolean);
      return {
        ...pageResult(items, matches.length, page, pageSize),
        search: ranked.meta,
      };
    }

    const terms = normalizeArabic(query.q)
      .split(' ')
      .filter((t) => t.length > 1)
      .slice(0, 5);
    if (terms.length)
  ...
  async detail(id: string) {
    ...
    if (!product)
      throw new NotFoundException('المنتج غير موجود أو لم يعد متاحاً');
  ...
  private assertBelowLimit(
    store: Awaited<ReturnType<ProductsService['storeOf']>>,
  ) {
  ...
  private async prepare(
    ...
    if (!onRequest && !dto.price)
      throw new BadRequestException('أدخل السعر أو اختر "السعر عند الطلب"');
    const price = onRequest ? null : dto.price!;
    const oldPrice =
      !onRequest && dto.oldPrice && price && dto.oldPrice > price
        ? dto.oldPrice
        : null;
  ...
  private assessRisk(p: ProductInputDto): {
    riskScore: number;
    ...
    if (banned.some((re) => re.test(text)))
      return { riskScore: 100, status: 'UNDER_REVIEW' };
    ...
    return {
      riskScore: score,
      status: score >= 50 ? 'UNDER_REVIEW' : 'ACTIVE',
    };
  }
}
