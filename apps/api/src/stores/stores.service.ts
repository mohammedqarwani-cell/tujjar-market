...
export class StoresService {
  ...
  async list(query: Record<string, string>) {
    ...
    if (ranked) {
      ...
      matches.sort(
        (a, b) => ranked.scores.get(b.id)! - ranked.scores.get(a.id)!,
      );
    ...
    if (query.open === '1') {
      ...
      const open = all.filter(
        (s) => openState(s.openingSchedule as never)?.open,
      );
      return pageResult(
        open.slice(skip, skip + take),
        open.length,
        page,
        pageSize,
      );
  ...
  async update(userId: string, dto: UpdateStoreDto) {
    ...
    if (dto.openingSchedule && !isValidSchedule(dto.openingSchedule)) {
      throw new BadRequestException(
        'ساعات العمل غير صحيحة، راجع أوقات الفتح والإغلاق',
      );
    }
    const hasPin =
    ...
    if (!whatsapp)
      throw new BadRequestException('رقم الواتساب غير صحيح، مثال: 0912345678');
    ...
    if (
      governorate.id !== store.governorateId &&
      governorate.status !== 'ACTIVE'
    ) {
      throw new BadRequestException(
        `التسجيل في ${governorate.name} يفتح قريباً`,
      );
    }
    ...
    const movedOrRenamed =
      store.name !== name ||
      store.governorateId !== governorate.id ||
      store.marketId !== (market?.id ?? null);
    const resetShopVerification =
      movedOrRenamed && atLeast(store.earnedLevel, 'LOCATION');
    ...
    await this.prisma.store.update({
      ...
      data: {
        ...
        ...(dto.latitude !== undefined || dto.longitude !== undefined
          ? {
              latitude: hasPin ? dto.latitude : null,
              longitude: hasPin ? dto.longitude : null,
            }
          : {}),
