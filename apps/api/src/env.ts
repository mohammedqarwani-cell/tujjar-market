...
function secret(name: string, minLength: number, devFallback: string): string {
  ...
  if (!value || value.length < minLength) {
    throw new Error(
      `Environment variable ${name} is missing or shorter than ${minLength} characters`,
    );
  }
  return value;
}
...
function aesKey(name: string, devFallback: string): Buffer {
  const key = Buffer.from(
    secret(name, 44, Buffer.from(devFallback).toString('base64')),
    'base64',
  );
  if (key.length !== 32)
...
if (cookiePathPrefix && !cookiePathPrefix.startsWith('/'))
  throw new Error('COOKIE_PATH_PREFIX must start with "/"');
...
if (publicBucket === privateBucket)
  throw new Error(
    'MINIO_PRIVATE_BUCKET must differ from the public MINIO_BUCKET',
  );
...
export const env = {
  ...
  jwtAccessSecret: secret(
    'JWT_ACCESS_SECRET',
    32,
    'dev-access-secret-change-me-0123456789abcdef',
  ),
  otpPepper: secret(
    'OTP_PEPPER',
    32,
    'dev-otp-pepper-change-me-0123456789abcdefgh',
  ),
  ...
  vapid: {
    ...
    subject:
      ...
      `mailto:${process.env.VAPID_CONTACT_EMAIL ?? 'support@tujjar.market'}`,
  },
  minio: {
