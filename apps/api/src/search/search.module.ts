...
export class SuggestService {
  ...
  async suggest(raw: string) {
    ...
    const [products, stores] = await Promise.all([
      this.index.rankProducts(q),
      Promise.resolve(this.index.rankStores(q)),
    ]);
    ...
    const [productRows, storeRows, categories] = await Promise.all([
      ...
      storeIds.length
        ? this.prisma.store.findMany({
            where: { id: { in: storeIds }, ...publicStoreWhere },
    ...
    const order = <T extends { id: string }>(rows: T[], ids: string[]) =>
      ids.map((id) => rows.find((r) => r.id === id)).filter((r): r is T => !!r);
    const terms = nq.split(' ').filter((t) => t.length >= 2);
    return {
      products: order(productRows, productIds).map((p) => ({
        id: p.id,
        title: p.title,
      ...
      categories: categories
        .filter((c) => terms.some((t) => normalizeArabic(c.name).includes(t)))
        .slice(0, 2),
