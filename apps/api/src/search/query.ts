...
const GROUPS = SYNONYM_GROUPS.map((g) => [
  ...new Set(g.map(norm).filter(Boolean)),
]);
...
const INTENT_RULES = INTENTS.map((r) => ({
  when: r.when.flat().map(norm),
  add: r.add.map(norm),
}));
...
GROUPS.forEach((g, i) =>
  g.forEach((w) => GROUP_OF.set(w, [...(GROUP_OF.get(w) ?? []), i])),
);
const PHRASES = [...GROUP_OF.keys()]
  .filter((w) => w.includes(' '))
...
export function stems(word: string): string[] {
  ...
  for (const p of PREFIXES) {
    if (word.startsWith(p) && word.length - p.length >= 3)
      out.add(word.slice(p.length));
  }
  for (const w of [...out]) {
    for (const s of SUFFIXES) {
      if (w.endsWith(s) && w.length - s.length >= 3)
        out.add(w.slice(0, -s.length));
    }
  }
  return [...out];
}
...
export function editDistance(a: string, b: string, max: number): number {
  ...
  for (let i = 1; i <= a.length; i++) {
    ...
    for (let j = 1; j <= b.length; j++) {
      const sub =
        a[i - 1] === b[j - 1] ? 0 : soft(a[i - 1]) === soft(b[j - 1]) ? 0.4 : 1;
      d[i][j] = Math.min(
        d[i - 1][j] + 1,
        d[i][j - 1] + 1,
        d[i - 1][j - 1] + sub,
...
function variantsFor(word: string, weight: number): Variant[] {
  ...
  for (const s of stems(word)) {
    ...
    for (const gi of GROUP_OF.get(s) ?? [])
      for (const syn of GROUPS[gi]) add(syn, weight * 0.9);
  }
...
export function parseQuery(raw: string, vocab: Vocabulary): ParsedQuery {
  ...
  for (const word of text.split(' ').filter(Boolean)) {
    ...
    if (MOD.offers.has(word)) {
      wantsOffers = true;
      continue;
    }
    if (MOD.used.has(word)) {
      condition = 'USED';
      continue;
    }
    if (MOD.new.has(word)) {
    ...
    if (fixed) {
      concepts.push({
        word,
        corrected: fixed,
        variants: variantsFor(fixed, 0.85),
      });
  ...
  const correctedText = concepts.some((c) => c.corrected)
    ? corrections.join(' ')
    : null;
  return {
    normalized,
