...
const MARKETING = (Object.keys(CATEGORY_INFO) as NotificationCategory[]).filter((c) => CATEGORY_INFO[c].marketing);
const MAX_SUBSCRIPTIONS_PER_USER = 10;
const CHUNK = 200;

const audienceOf = (role: Role): Audience =>
  (Object.keys(ROLE_AUDIENCE) as Audience[]).find((aud) => ROLE_AUDIENCE[aud].includes(role)) ?? 'web';
...
export class NotificationsService {
  ...
  notify(userIds: string | string[] | null | undefined, input: NotificationInput) {
    const ids = (Array.isArray(userIds) ? userIds : [userIds]).filter((id): id is string => !!id);
    if (!ids.length) return;
    void this.deliver(ids, input).catch((e) => this.log.error(`Notification ${input.type} failed`, (e as Error).stack));
  }

  /** Moderators and admins (moderation queues). */
  notifyStaff(input: NotificationInput) {
    void this.prisma.user
      .findMany({ where: { role: { in: ['ADMIN', 'MODERATOR'] }, status: 'ACTIVE' }, select: { id: true } })
      .then((users) => this.notify(users.map((u) => u.id), input))
      .catch((e) => this.log.error(`Staff notification ${input.type} failed`, (e as Error).stack));
  }

  async deliver(userIds: string[], input: NotificationInput): Promise<{ delivered: number; pushed: number }> {
    ...
    for (let i = 0; i < ids.length; i += CHUNK) {
      ...
      for (let j = 0; j < users.length; j += 20) {
        const results = await Promise.all(users.slice(j, j + 20).map((u) => this.deliverOne(u, input)));
  ...
  private async deliverOne(
    ...
    const prefs = effectivePrefs(user.role, user.notificationPrefs, input.category);
    ...
    if (existing) {
      ...
      await this.prisma.notification.update({
        ...
        data: { count, title, body, url: input.url ?? null, silent, createdAt: now },
      });
    ...
    if (existing?.pushedAt && now.getTime() - existing.pushedAt.getTime() < GROUP_REPUSH_MS) {
      return { shown: !silent, pushed: false };
    }
    if (CATEGORY_INFO[input.category].marketing && !(await this.marketingPushAllowed(user.id, now))) {
      return { shown: !silent, pushed: false };
    }
    ...
    const sent = await this.push.send(targets, { title, body, url: input.url, tag: input.groupKey ?? notificationId }, input.urgent);
    if (sent) await this.prisma.notification.update({ where: { id: notificationId }, data: { pushedAt: now } });
  ...
  private async marketingPushAllowed(userId: string, now: Date) {
    ...
    if (hour < MARKETING_PUSH_HOURS.from || hour >= MARKETING_PUSH_HOURS.to) return false;
    const recent = await this.prisma.notification.count({
      where: { userId, category: { in: MARKETING }, pushedAt: { gte: new Date(now.getTime() - 24 * 3600_000) } },
    });
  ...
  async list(userId: string, query: Record<string, string>) {
    ...
    const [items, total, unread] = await this.prisma.$transaction([
      this.prisma.notification.findMany({ where, select: listSelect, orderBy: { createdAt: 'desc' }, skip, take }),
  ...
  private unreadQuery(userId: string) {
    return this.prisma.notification.count({ where: { userId, silent: false, readAt: null } });
  }
  ...
  async markRead(userId: string, ids?: string[]) {
    await this.prisma.notification.updateMany({
      where: { userId, readAt: null, ...(ids?.length ? { id: { in: ids.slice(0, 100) } } : {}) },
  ...
  async preferences(userId: string) {
    const user = await this.prisma.user.findUniqueOrThrow({ where: { id: userId }, select: { role: true, notificationPrefs: true } });
    return {
  ...
  async updatePreferences(userId: string, changes: Record<string, { inApp?: boolean; push?: boolean }>) {
    const user = await this.prisma.user.findUniqueOrThrow({ where: { id: userId }, select: { role: true, notificationPrefs: true } });
    const allowed = ROLE_CATEGORIES[user.role];
    const next: PrefsMap = { ...((user.notificationPrefs as PrefsMap | null) ?? {}) };
    ...
    await this.prisma.user.update({ where: { id: userId }, data: { notificationPrefs: next as Prisma.InputJsonValue } });
  ...
  async subscribe(
    ...
    const data = { userId, audience, p256dh: sub.keys.p256dh, auth: sub.keys.auth, userAgent, failures: 0 };
    // A device signing in to another account moves its subscription to that account
    await this.prisma.pushSubscription.upsert({ where: { endpoint: sub.endpoint }, create: { endpoint: sub.endpoint, ...data }, update: data });
    ...
    if (extra.length) await this.prisma.pushSubscription.deleteMany({ where: { id: { in: extra.map((s) => s.id) } } });
