...
const INELIGIBLE_MESSAGES: Record<Ineligible, string> = {
  ...
  NO_CONTACT:
    'التقييم متاح لمن تواصل مع المتجر عبر واتساب أو اتصال من صفحته في تُجّار ماركت',
...
const HOLD_RULES: { pattern: RegExp; note: string }[] = [
  {
    pattern: /(https?:\/\/|www\.|t\.me\/|wa\.me\/|\.(com|net|org|sy)\b)/i,
    note: 'يحتوي رابطاً',
  },
...
export class ReviewsService {
  ...
  async list(slug: string, query: Record<string, string>) {
    ...
    return {
      ...pageResult(
        items.map(({ buyer, ...review }) => ({
          ...review,
          author: maskName(buyer.name),
        })),
        total,
        page,
        pageSize,
  ...
  private async ineligibility(
    ...
    if (
      buyer &&
      [store.whatsapp, store.phone, store.owner.phone].includes(buyer.phone)
    ) {
      return { reason: 'OWN_STORE', availableAt: null };
    }
    if (!contact) return { reason: 'NO_CONTACT', availableAt: null };
    if (
      contact.lastContactAt.getTime() <
      Date.now() - REVIEW_CONTACT_VALID_DAYS * DAY_MS
    ) {
      return { reason: 'CONTACT_TOO_OLD', availableAt: null };
    }
    const availableAt = new Date(
      contact.firstContactAt.getTime() + REVIEW_CONTACT_COOLDOWN_MS,
    );
    if (availableAt.getTime() > Date.now())
      return { reason: 'TOO_SOON', availableAt };
  ...
  async mine(buyerId: string, slug: string) {
    ...
    const canReview =
      check.reason === null || (!!review && check.reason !== 'OWN_STORE');
    return {
      canReview,
      reason: canReview ? null : check.reason,
      message:
        canReview || !check.reason ? null : INELIGIBLE_MESSAGES[check.reason],
  ...
  async submit(buyerId: string, slug: string, dto: ReviewInputDto, ip: string) {
    ...
    if (check.reason && !(existing && check.reason !== 'OWN_STORE')) {
      throw new ForbiddenException({
        statusCode: 403,
        message: INELIGIBLE_MESSAGES[check.reason],
        code: check.reason,
      });
    }
    ...
    const hold = comment
      ? HOLD_RULES.find((rule) =>
          rule.pattern.test(
            `${toLatinDigits(comment)} ${normalizeArabic(comment)}`,
          ),
        )
      : undefined;
    ...
    const moderationNote = hold
      ? `محجوز تلقائياً: ${hold.note}`
      : reheld
        ? 'عُدّل بعد إخفائه من الإدارة'
        : null;
    ...
    if (status === 'PUBLISHED' && !existing)
      this.announceReview(store.id, dto.rating);
    if (status === 'UNDER_REVIEW') {
      this.notifications.notifyStaff({
        ...
        grouped: (count) => ({
          title: 'تقييمات تنتظر المراجعة',
          body: `${count} تقييمات محجوزة اليوم`,
        }),
      });
    }
    return review;
    ...
    void this.prisma.store
      ...
      .then((store) =>
        this.notifications.notify(store?.ownerId, {
          ...
          grouped: (count) => ({
            title: 'تقييمات جديدة لمتجرك',
            body: `وصلك ${count} تقييمات جديدة اليوم`,
          }),
        }),
      );
  }
  ...
  async merchantList(userId: string, query: Record<string, string>) {
    ...
    if (['PUBLISHED', 'UNDER_REVIEW', 'HIDDEN'].includes(query.status))
  ...
  async reply(userId: string, id: string, reply: string, ip: string) {
    ...
    if (review.status !== 'PUBLISHED')
      throw new BadRequestException('يمكن الرد على التقييمات المنشورة فقط');
  ...
  async flag(userId: string, id: string, reason: string, ip: string) {
    ...
    if (review.flagOpen)
      throw new ConflictException(
        'طلبت مراجعة هذا التقييم مسبقاً، وهو لدى الإدارة',
      );
    ...
    this.notifications.notifyStaff({
      ...
      grouped: (count) => ({
        title: 'تقييمات تنتظر المراجعة',
        body: `${count} تقييمات بانتظار قرار اليوم`,
      }),
    });
  ...
  async adminList(query: Record<string, string>) {
    ...
    const status = (
      ['PUBLISHED', 'UNDER_REVIEW', 'HIDDEN'].includes(query.status)
        ? query.status
        : 'UNDER_REVIEW'
    ) as ReviewStatus;
    const where: Prisma.ReviewWhereInput =
      query.flagged === '1' ? { flagOpen: true } : { status };
    const [items, total] = await this.prisma.$transaction([
      this.prisma.review.findMany({
        where,
        select: {
    ...
    const contacts = items.length
      ? await this.prisma.storeContact.findMany({
          where: {
            OR: items.map((r) => ({ storeId: r.storeId, buyerId: r.buyerId })),
          },
          select: {
            storeId: true,
            buyerId: true,
    ...
    const withContact = items.map(({ storeId, buyerId, ...review }) => {
      const contact = contacts.find(
        (c) => c.storeId === storeId && c.buyerId === buyerId,
      );
      return {
        ...review,
        contact: contact
  ...
  async moderate(
    ...
    const review = await this.prisma.review.findUnique({
      ...
      select: {
        ...
        flagOpen: true,
        buyerId: true,
        store: { select: { ownerId: true, name: true, slug: true } },
      },
    });
    if (!review) throw new NotFoundException('التقييم غير موجود');
    ...
    if (review.flagOpen) {
      this.notifications.notify(review.store.ownerId, {
        ...
        body:
          dto.status === 'HIDDEN'
            ? 'أُخفي التقييم بعد المراجعة'
            : 'بقي التقييم منشوراً بعد المراجعة',
        url: '/dashboard/reviews',
        urgent: true,
      });
    }
  ...
  pendingCount() {
    return this.prisma.review.count({
      where: { OR: [{ status: 'UNDER_REVIEW' }, { flagOpen: true }] },
    });
  }
}
